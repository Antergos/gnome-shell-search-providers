/* TheMovieDB Search Provider for TvShows for Gnome Shell
 *
 *
...
const Gettext = imports.gettext.domain('themoviedb_tvshows_search_provider');
const _ = Gettext.gettext;

const CACHE_FOLDER = '.cache/antergos-search-providers-gnome/tvshows';

const TheMovieDBTvShowsSearchProvider = new Lang.Class({
    Name: 'TheMovieDBTvShowsSearchProvider',
    ...
    _init: function() {
        ...
        this.appInfo.get_name = function() {
            return _('TheMovieDB Search Provider for TvShows');
        };
    ...
    getInitialResultSet: function(terms, callback, cancellable) {
        ...
        if (terms.length >= 2 && terms[0].substr(0, 2) === 'tv') {
    ...
    _cachePoster: function (object, image_name, url, callback) {
        ...
        if(url && typeof url === 'string' && image_name !== '') {
            ...
            if(!file.query_exists(null)) {
                ...
                return new St.Icon({ gicon: Gio.icon_new_for_string(this._home_path + '/' + CACHE_FOLDER + image_name), style_class: 'themoviedb-search-provider-poster'});
            }else {
...
let theMovieDBTvShowsSearchProvider = null;
...
function enable() {
    if (!theMovieDBTvShowsSearchProvider) {
        theMovieDBTvShowsSearchProvider = new TheMovieDBTvShowsSearchProvider();
        Main.overview.viewSelector._searchResults._registerProvider(
            theMovieDBTvShowsSearchProvider
        );
    }
}
...
function disable() {
    if (theMovieDBTvShowsSearchProvider){
        ...
        theMovieDBTvShowsSearchProvider = null;
    }
}
